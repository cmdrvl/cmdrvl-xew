...
- contains an observable pattern with a specific, testable break mechanism that could be triggered by any of: (a) disclosure structure change, (b) taxonomy change, or (c) changes in EDGAR validation/enforcement behavior.
...
Form-specific comparator policy (v1):
- 10-Q, 10-K, 20-F: compare to the prior filing of the same base form (latest /A if present).
- 6-K, 8-K: no comparator by default; only compare if an explicit comparator is provided.

Comparator notes for additional forms (draft; lock before enabling):
- Base form type is the form with any "/A" stripped.
- 20-F compares to the prior 20-F (or 20-F/A if it amends the prior period).
- 6-K and 8-K are non-periodic; comparator-based checks are disabled by default. If enabled, select the most recent same base form within a configurable lookback window, and record the policy (window + selection rule) in the Evidence Pack.
...
- Structurally questionable XBRL patterns may pass validation for years, then fail suddenly when enforcement tightens or disclosure structure changes.
...
- Adaptation markers derived from filing history improve triage clarity compared to alerts without historical context (markers help distinguish "stable filers" from "recently migrated filers") without being treated as causal proof.
...
Marker output payload (recorded in `xew_findings.json`):
- `marker_id`: marker identifier (e.g., `XEW-M002`)
- `boundary`: `from_accession` → `to_accession` where the change-point is detected
- `evidence`: structured fields used to justify the marker (counts, ratios, thresholds, sample identifiers)
- Any numeric thresholds used must be recorded (and also captured in toolchain config) for reproducibility.

Suggested `evidence` fields (v1 planning):
- `metric`: name of the measured change (e.g., `extension_concept_churn`)
- `from_value` / `to_value`: numeric values on each side of the boundary
- `delta` or `ratio`: computed change across the boundary
- `threshold`: numeric threshold used to trigger the marker
- `sample_accessions`: short list of accession IDs used to compute the change
...
Threshold rule (v1): the descriptions above use qualitative terms ("materially", "high churn", "sharply") as placeholders. At implementation time, these must be replaced with explicit numeric thresholds (e.g., ">30% change in extension concept count") and recorded in the Evidence Pack/toolchain config so results are reproducible. Markers without pinned thresholds must not ship.

## V1 Shipping Pattern Set (Scope Lock)
In v1, CMD+RVL generates alert candidates only for the following target shipping patterns (subject to the Gate), listed in alert priority order (highest first):
1. `XEW-P001` Duplicate Facts With Equivalent Context/Unit (rule basis required) — highest priority because duplicates are unambiguous structural errors
2. `XEW-P006` Fee Exhibit Fragility (objective checks; rule basis required) — flags fee-exhibit warning conditions being reclassified as suspension-level errors effective March 16, 2026
3. `XEW-P004` Invalid or Discouraged Data-Type / Unit / Numeric Attributes (rule-basis required) — objective type/unit violations
4. `XEW-P005` Inconsistent Taxonomy References (rule-basis required; focus on internal inconsistency in v1) — namespace/schemaRef mismatches
5. `XEW-P002` Extension Anchoring Defects (objective checks only; rule basis required; no "semantic unrelatedness" claims) — often requires more context to assess severity

When multiple patterns are detected in one filing and only one alert is sent (the default), select the highest-priority pattern.
...
- Why fragile: Per XBRL semantics, two facts with identical concept/context/unit represent the same assertion and are considered duplicates. Duplicates create ambiguity for consumers (which value is authoritative?); they may be tolerated in stable filings but can become reject-risk under enforcement tightening (e.g., stricter EFM validation) or disclosure evolution that surfaces the ambiguity.
...
- Signal: the same concept is reported both with dimensions and without dimensions (or with materially different dimension-member sets) in contexts that should be comparable (same period type and similar business meaning).
...
- Research notes (non-shipping; objective signals to explore):
  - Same concept QName appears with and without any dimensions within the same period type and same presentation role (record role URI + counts).
  - Same concept QName appears with multiple distinct dimension-member sets in contexts that share entity + period (record distinct dim-sig count).
  - Same concept QName appears across multiple hypercubes where the primary item set is identical but dimensional coverage differs (record hypercube/role IDs).
  - Candidate metrics to capture for review: number of distinct dim-sigs per concept, ratio of dimensional vs non-dimensional instances, number of roles/hypercubes involved.
...
- Signal: taxonomy references are internally inconsistent within the same filing (e.g., extension schema imports `http://fasb.org/us-gaap/2024` but facts use `http://fasb.org/us-gaap/2025` namespace), or schemaRef hrefs don't match the namespaces actually used in facts.
...
- v1 status: ship (objective checks; rule basis required; time-sensitive enforcement shift effective March 16, 2026).
- Signal: an accepted filing's structured filing-fee exhibit contains objective fee-exhibit warning conditions on rules that EDGAR has announced will be enforced as suspension-level errors starting March 16, 2026.
- Why fragile: the artifact was accepted under a warning-tolerant regime; the same defect becomes submission-blocking under the announced enforcement shift.
- Break trigger examples: validator/enforcement tightening (fee-exhibit warnings → suspensions effective 2026-03-16).
- Evidence must include: the fee exhibit artifact bytes + sha256; the objective condition detected (e.g., missing required FFD facts); the mapped SEC error ID/code (when known); and a two-layer rule basis (underlying rule + reclassification announcement) pinned in the Evidence Pack.
...
- Research notes (non-shipping; objective signals to explore):
  - Multiple contexts share identical entity + period + dimension-member signatures but have disjoint fact sets (record context IDs + fact counts).
  - Duplicate-by-substance contexts appear across multiple presentation roles for the same primary items (record role URIs + counts).
  - Candidate metrics to capture for review: number of duplicate context groups, max contexts per group, total facts per group.
...
Commands (v1):
- `cmdrvl-xew flatten`: normalize EDGAR directory structure (with typed subdirectories like EX-101.SCH/) into a flat layout that Arelle can load.
- `cmdrvl-xew pack`: generate an Evidence Pack from flat artifacts (output of `flatten` or pre-staged flat directory).
...
EDGAR-driven fetch policy (draft requirements):
- User-Agent must be configurable and include contact information (company + email) and tool version.
- Rate limiting must be configurable (max requests/sec and/or concurrency) and honor SEC guidance at time of implementation.
- Respect server signals: handle 429/503 with exponential backoff and honor Retry-After when present.
- Cache responses to minimize requests and record retrieval metadata (URL, retrieved_at, status, sha256) in the Evidence Pack.
- Do not enable EDGAR fetch mode without this policy implemented and documented.
...
- Break trigger (explicit): `XEW-BT###` + the break trigger most directly connected to the observed pattern (selected from the Break Trigger Catalog; when multiple apply, pick the one most specific to the pattern)
...
- Product surfaces (outside this repo) that host monitoring, alert delivery, and Evidence Pack access
...
- Produce 3-5 internal alerts end-to-end with Evidence Packs (human reviewed); if insufficient patterns are detected in pilot issuers, expand the issuer set or backtest against historical filings known to have issues
...
## Future Work Gating Criteria (CLI Roadmap)
Future items are not eligible for v1 unless they meet explicit gates:
- EDGAR-driven fetch mode: SEC-compliant User-Agent + rate limiting, deterministic artifact selection, and recorded retrieval metadata.
- Severity tiers (v2 schema): versioned schema update plan, no v1 output changes, and a migration note in the contract.
- Expanded form coverage (20-F/6-K/8-K): ingestion rules + comparator policy per form, plus golden Evidence Pack fixtures.
- Roadmap patterns (P003/P007): canonical signature definitions, noise evaluation plan, and log-only behavior before alerting.
